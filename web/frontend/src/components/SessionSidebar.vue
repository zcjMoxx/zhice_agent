...
function beginRename(id: string, title: string) { renameId.value = id; renameTitle.value = title || tr("新对话", "New chat"); actionSession.value = ""; }
...
<template>
      ...
      <button v-for="session in sessions.filtered" :key="session.session_id" class="session-row" :class="{ active: sessions.activeId === session.session_id }" @click="sessions.open(session.session_id)">
        ...
        <span class="session-actions">
          ...
          <span v-if="actionSession === session.session_id" class="popover session-menu" @click.stop>
            ...
            <button class="danger-text" @click="deleteId = session.session_id; actionSession = ''">{{ tr('删除', 'Delete') }}</button>
          </span>
        </span>
      </button>
  ...
  <div v-if="renameId" class="modal-backdrop" @click.self="renameId = ''">
    <form class="dialog-card compact-dialog" @submit.prevent="saveRename"><h2>{{ tr('重命名 Session', 'Rename Session') }}</h2><input v-model="renameTitle" maxlength="120" autofocus required /><p v-if="failure" class="form-error">{{ failure }}</p><div class="dialog-actions"><button type="button" @click="renameId = ''">{{ tr('取消', 'Cancel') }}</button><button class="primary-button">{{ tr('保存', 'Save') }}</button></div></form>
  </div>
  <div v-if="deleteId" class="modal-backdrop" @click.self="deleteId = ''">
    <section class="dialog-card compact-dialog"><h2>{{ tr('删除这个 Session？', 'Delete this Session?') }}</h2><p>{{ tr('消息与相关 Session 元数据将被删除，此操作无法撤销。', 'Messages and related Session metadata will be deleted. This cannot be undone.') }}</p><div class="dialog-actions"><button @click="deleteId = ''">{{ tr('取消', 'Cancel') }}</button><button class="danger-button" @click="confirmDelete">{{ tr('确认删除', 'Delete') }}</button></div></section>
  </div>
</template>
