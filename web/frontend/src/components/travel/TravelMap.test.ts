...
describe("TravelMap", () => {
  ...
  it("keeps the textual route available when the map key is absent", async () => {
    ...
    expect(wrapper.text()).toContain("仍保留当天地点顺序、交通方式、距离和时长");
  ...
  it("geocodes missing historical locations and draws map overlays", async () => {
    ...
    const Map = vi.fn(function Map() { return { add, clearMap, setFitView, destroy: vi.fn() }; });
    ...
    expect(wrapper.text()).toContain("已为当天历史计划动态补全 1 个地点坐标");

    await wrapper.findAll(".travel-map-day-tabs button")[1].trigger("click");
    await vi.waitFor(() => expect(Marker).toHaveBeenCalledTimes(2));

    expect(clearMap).toHaveBeenCalledTimes(2);
    expect(wrapper.get(".travel-map-itinerary").text()).toContain("洱海");
    expect(wrapper.get(".travel-map-itinerary").text()).not.toContain("10-01 · 大理");
    const markerCalls = Marker.mock.calls as unknown as Array<[Record<string, unknown>]>;
    expect(markerCalls[1]?.[0]).toMatchObject({ title: "1. 洱海" });
  });
});
...
function sampleDays(): TravelDay[] {
  return [{
    ...
    date: "2026-10-02",
    city_or_area: "洱海",
    activities: [{ start: "09:00", end: "11:00", place: "洱海", reason: "骑行", evidence_ids: [], location: { longitude: 100.20, latitude: 25.75 } }],
    route_segments: [{ mode: "公交", from: "古城", to: "洱海", duration: 30, distance: 8, source: "高德", evidence_ids: [], path: [] }],
    meal_suggestions: [],
    daily_budget: 300,
    weather_adjustment: "",
    fallback_plan: "",
