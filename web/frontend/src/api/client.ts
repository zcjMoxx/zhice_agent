...
export const api = {
  ...
  bindings: () => request<{ bindings: Array<{ binding_id: string; channel: string; display_name: string; linked_at: string }> }>("/api/channels/bindings"),
  ...
  weixinStatus: () => request<{ status: string; linked_at: string }>("/api/channels/weixin", { cache: "no-store" }),
  startWeixin: () => request<{ attempt_id: string; status: string; expires_at: string; qr_data: string; error_code: string }>("/api/channels/weixin/binding-attempts", { method: "POST", cache: "no-store" }),
  pollWeixin: (id: string) => request<{ attempt_id: string; status: string; expires_at: string; qr_data: string; error_code: string }>(`/api/channels/weixin/binding-attempts/${encodeURIComponent(id)}`, { cache: "no-store" }),
  cancelWeixin: (id: string) => request(`/api/channels/weixin/binding-attempts/${encodeURIComponent(id)}`, { method: "DELETE", cache: "no-store" }),
