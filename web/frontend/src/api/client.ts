...
export const api = {
  ...
  xhsAdminStatus: () => request<XhsReadonlyAdminStatus>("/api/admin/external-platforms/xhs/status", { cache: "no-store" }),
  checkXhsAdminLogin: () => request<XhsReadonlyAdminStatus>("/api/admin/external-platforms/xhs/check-login", { method: "POST", cache: "no-store" }),
  ...
  travelProgress: (sessionId: string) => request<TravelProgressHistory>(`/api/travel/sessions/${encodeURIComponent(sessionId)}/progress`, { cache: "no-store" }),
  ...
  workflows: () => request<{ items: import("./types").WorkflowSummary[] }>("/api/workflows", { cache: "no-store" }),
  ...
  requestNotificationEmailVerification: (address: string) => request<{ challenge_id: string; address: string; expires_at: string; retry_after_seconds: number }>("/api/connections/email/notification/request-verification", json({ address })),
  ...
  testWorkflowTool: (name: string, argumentsValue: Record<string, unknown>) => request<{ status: string; output: unknown }>("/api/workflow-tools/test", json({ name, arguments: argumentsValue })),
  workflow: (id: string) => request<WorkflowDetail>(`/api/workflows/${encodeURIComponent(id)}`, { cache: "no-store" }),
  createWorkflow: (definition: WorkflowDefinitionV1) => request<WorkflowDetail>("/api/workflows", json(definition)),
  saveWorkflowDraft: (id: string, definition: WorkflowDefinitionV1, revision?: number) => request<WorkflowDetail>(`/api/workflows/${encodeURIComponent(id)}/draft`, { method: "PUT", body: JSON.stringify({ ...definition, expected_version: revision }) }),
  publishWorkflow: (id: string) => request<WorkflowDetail>(`/api/workflows/${encodeURIComponent(id)}/publish`, { method: "POST" }),
  ...
  deleteWorkflow: (id: string) => request<{ status: string }>(`/api/workflows/${encodeURIComponent(id)}`, { method: "DELETE" }),
};
