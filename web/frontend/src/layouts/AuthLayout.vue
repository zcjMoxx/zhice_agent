...
const panelTitle = computed(() => {
  ...
  return mode.value === "login" ? tr("欢迎回来", "Welcome back") : tr("创建本地账号", "Create a local account");
});
    ...
    <section class="auth-slider" :class="{ 'is-register': mode === 'register' && !setup, 'is-setup': setup, 'is-channel-binding': flow === 'qq-binding' }">
      ...
      <form class="auth-form-panel" @submit.prevent="submit">
        <div class="auth-form-heading">
          ...
          <div><h2>{{ panelTitle }}</h2><p>{{ setup ? tr('Owner 用户名固定为 owner', 'The Owner username is fixed as owner') : flow === 'qq-binding' ? tr('完成后会自动绑定，无需再进入设置。', 'QQ will connect automatically after authentication.') : mode === 'login' ? tr('登录你的 ZhiCe-Agent 账号', 'Sign in to your ZhiCe-Agent account') : tr('新账号默认拥有普通用户角色', 'New accounts receive the standard user role') }}</p></div>
        </div>
        ...
        <button class="primary-button auth-submit" :disabled="busy" type="submit">{{ busy ? tr('处理中…', 'Working…') : setup ? tr('创建 Owner 并登录', 'Create Owner and sign in') : flow === 'qq-binding' ? mode === 'login' ? tr('登录并继续', 'Sign in and continue') : tr('创建账号并继续', 'Create account and continue') : mode === 'login' ? tr('登录', 'Sign in') : tr('创建并登录', 'Create and sign in') }}</button>
