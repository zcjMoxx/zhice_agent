...
第十四部分实现二微信 ClawBot 已落地。当前口径已经合并进 `zhice-agent-part14-external-channel-design.md`，完整取舍和真实 POC 证据见 `2026-07-24-weixin-clawbot-channel-design.md`。一个已登录 Web 用户最多连接一个微信 AI 账号；多个账号由同一个微信专用 Node Transport sidecar 维护，Agent、Session、Memory、Tool 和权限仍只运行在 ZhiCe-Agent。2026-07-24 已验证真实 AI 标识、扫码、direct text 收发、游标/context token 恢复与 notifyStop；双真实账号并发仍需第二名用户验收。生产部署代码已随 Part 17 落地，真实镜像与云端验收仍待 Docker daemon 可用后关闭。
...
Part 17 运行可靠性、系统级诊断与私有镜像部署的代码与测试已进入当前基线。当前活文档见 `zhice-agent-part17-reliability-diagnostics-deployment-design.md`，完整方案与实施说明见 `2026-07-29-part17-runtime-reliability-diagnostics-and-deployment-design.md`。Provider 结构化错误/重试/cooldown/attempts、系统诊断、MCP 动态刷新、重启恢复、单 worker 边界和 `deploy/` 私有覆盖层均已落地；默认 workspace 统一为 `Path.home() / ".zhice"`（Windows `C:\Users\<user>\.zhice`，Docker `/home/zhice/.zhice`），运行态 env 位于 `${workspace}/config/.env`，解析优先级为 `--workspace > ZHICE_AGENT_WORKSPACE > 默认目录`。普通 `zcagent init` 默认补齐 `config/.env`，已有文件保留、`--force` 覆盖，`--write-env` 仅作兼容参数。最新全量 Python `796 passed, 1 skipped`，Ruff、前端 `29` 项测试、lint/typecheck/build、deploy 静态检查与 compose 校验通过。当前 Docker Desktop daemon 未运行，真实 image build/run smoke、registry push 和云端 deploy 尚未执行，因此 Part 17 实现完成但生产部署验收未关闭。
