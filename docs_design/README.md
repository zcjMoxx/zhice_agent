...
- `zhice-agent-part9-user-auth-permission-design.md`：Part 9，用户、登录与权限执行边界设计；普通自助注册默认关闭并由 Owner 独占控制，前端展示和后端注册接口使用同一持久策略。
...
- `zhice-agent-part20-visual-workflow-scheduler-design.md`：Milestone 20 已完成的当前活文档；独立 Workflow Runtime、SQLite、单实例 APScheduler、固定节点、草稿试运行、用户连接、已验证通知邮箱、本人 QQ/微信、受限 Node-RED、REST API、RuntimeEvent 与响应式 Vue Flow 页面基线。

### 近期实施记录（留痕）

以下带日期文件只用于理解近期实现收口过程，不作为当前事实来源：
...
- `2026-08-26-weather-discovery-and-chat-history-presentation-fix-design.md`：天气 Tool 发现与聊天历史展示补丁；当前天气关键词可以命中 Open-Meteo Tool，恢复历史时不再重复展示已经持久化的运行细节。
- `2026-08-26-cloud-deployment-smoke-opt-in-design.md`：云部署 smoke 改为显式启用；默认发布只执行必要启动步骤，只有 `-Smoke` 才等待 readiness/health 并执行核心工作流、外部集成和公网验证。
- `2026-08-26-ctrip-login-state-convergence-design.md`：携程登录态检查与账号操作收敛；启动时仅复检已有 profile，服务器使用 headless Chromium，管理页区分技术就绪与真实登录态。
- `2026-08-26-travel-finalizer-transport-source-reconciliation-design.md`：旅行终稿交通来源应急归一记录；当前整体实现已由服务端 revision 草稿与批量 repair 取代逐字段重提完整计划。
- `2026-08-26-travel-finalizer-server-draft-repair-design.md`：旅行终稿服务端规范草稿与整体修复；首次失败后持久化 revision，后续只提交有界 JSON Pointer repairs，并使用 optimistic concurrency 跨 Turn/重启收敛。

### 当前基线说明

Part 18 后进入特色应用阶段。Milestone 19 智能旅行规划和 Milestone 20 拖拽工作流均已完成代码落地；当前事实分别以 Part 19、Part 20 活文档为准。旅行及其它真实外部 smoke 仍需显式凭据、测试账号与服务登录态执行，不能用 Provider accepted 或容器 healthy 代替最终业务结果。云发布默认不执行 readiness/health、核心工作流、外部集成或公网 smoke，只有显式 `-Smoke` 才执行完整验证与核心失败回滚。
...
第十二部分生命周期事件与 Hook Runtime 已实现并关闭。当前实现口径以 `zhice-agent-part12-hooks-design.md` 为准，最终边界与取舍记录见 `2026-07-20-hook-runtime-boundary-design.md`，单 Hook 显式角色/权限豁免作用域见 `2026-07-21-hook-role-scope-design.md`。Part 18 已在独立边界实现 SkillExecutor、`skill.*` 和 ProgressSink；它们不是 Part 12 的 Hook 职责，也不从 `exec.command` 推断。

第十三部分并行 Subagent 编排已经实现并进入当前代码基线。当前实现口径以 `zhice-agent-part13-subagent-design.md` 为准，边界取舍记录见 `2026-07-21-subagent-runtime-boundary-design.md`，启动能力分级与诊断证据闭环见 `2026-07-21-startup-capability-and-subagent-diagnostics-design.md`。主 Agent 默认直接完成简单任务；只有并行、上下文隔离、专业能力或独立复核收益明确时，才通过批量 `delegate_tasks` 在同一 Turn 内并行运行 child，再 fan-in 返回稳定、有界且允许 partial 的结果供父 Agent 归纳。`/subagent` 的 `auto/off/once` 使用 Session sidecar 真值和原子 one-shot 消费；child 使用独立 AgentLoop、内部 Session、RuntimeEvent scope 和取消 token，以新鲜 child Session 开始任务，但继承父 Turn 的 failover-safe ContextBudget。Tool/Skill/MCP 能力经过父可见集合、Profile allow/deny 与内核 deny 三重收窄；可写任务进入独立 worktree，共享状态任务进入进程级 shared-exclusive lane。现有 RBAC、确认、Hook、workspace guard、MCP artifact 与审计链保持不变。当前上下文统一使用 Part 15 的预算内完整历史、确定性历史查询、结构化 compaction 与混合检索，不再采用“最近 3 + 旧相关最多 3”的早期基线。

第十四部分外部渠道第一版已经实现并进入当前代码基线。当前方案以 `zhice-agent-part14-external-channel-design.md` 为准，初始边界取舍记录见 `2026-07-23-qq-external-channel-boundary-design.md`，跨渠道 Session、用户自助解绑和 QQ Markdown 收敛见 `2026-07-23-cross-channel-session-binding-and-qq-markdown-design.md`，群聊手动一次性码边界见 `2026-07-24-qq-group-manual-binding-design.md`，群聊回复归属见 `2026-07-24-qq-group-reply-attribution-design.md`，真实客户端的 Markdown 引用兼容修复见 `2026-07-24-qq-group-markdown-reference-compatibility-fix.md`，共享纯文本展示与 QQ 回复序号见 `2026-07-24-plain-text-presentation-and-qq-reply-sequence-design.md`，出站投递确认见 `2026-07-24-qq-outbound-delivery-confirmation-design.md`，Session 清空命令统一改名见 `2026-07-24-clear-session-command-rename-design.md`，公网部署绑定地址修复见 `2026-08-04-qq-public-binding-url-deployment-fix.md`，微信扫码凭据的容器持久化修复见 `2026-08-06-weixin-credential-volume-design.md`。第一条真实渠道选择 QQ：运行态使用官方 Python SDK 的 WebSocket 连接；渠道层已经建立中性事件、能力声明、外部身份绑定、conversation route、持久去重、per-conversation 串行、附件 guard 和 RuntimeEvent 出站渲染。Web/CLI 作为私有控制面可见本人跨渠道历史，QQ 私聊可跨端继续，QQ群聊在 Web 只读并通过派生新 Web Session 继续；外部入口不能反向管理其它渠道 Session。QQ 裸 `/bind` 直接使用账号级 `web_base_url`；本地缺省仍为 `127.0.0.1`，公网启用时每个账号必须显式配置来自私有部署配置的 HTTPS origin。QQ 群聊 Runtime 回答经共享 renderer 转为可读纯文本并使用递增 `msg_seq`，CLI 复用同一 renderer；QQ 私聊继续按内容选择 Markdown。QQ Transport 只有在平台返回有效响应时才确认发送成功，botpy 返回 `None` 时记录 `send_unconfirmed` 并将 receipt 标记为 error，且不盲目重试同一 `msg_id + msg_seq`。CLI、Web、external WebSocket 与 QQ 当前统一使用 `/clear` 清空当前 Session，旧 `/reset` 不再执行清空。

第十四部分实现二微信 ClawBot 已落地。当前口径已经合并进 `zhice-agent-part14-external-channel-design.md`，完整取舍和真实 POC 证据见 `2026-07-24-weixin-clawbot-channel-design.md`。一个已登录 Web 用户最多连接一个微信 AI 账号；多个账号由同一个微信专用 Node Transport sidecar 维护，Agent、Session、Memory、Tool 和权限仍只运行在 ZhiCe-Agent。2026-07-24 已验证真实 AI 标识、扫码、direct text 收发、游标/context token 恢复与 notifyStop；双真实账号并发仍需第二名用户验收。生产镜像与真实云端运行已随 Part 17 完成验收，镜像中的微信 sidecar 能正常启动并报告渠道 ready。

原可靠性路线中的 Capability Selection 子能力已提前完成，设计记录见 `2026-07-21-on-demand-tool-discovery-design.md`。当前 CLI/Web/child Turn 首轮只暴露 `discover_tools`，发现后下一 LLM 步只增加已激活 Tool schema；Catalog 先经过 actor/Profile 过滤，未激活 dispatch fail closed。Provider retry、系统级诊断和 MCP reload 已随 Part 17 进入当前代码基线。
...
22. `zhice-agent-part20-visual-workflow-scheduler-design.md`
23. `2026-08-26-travel-finalizer-server-draft-repair-design.md`
24. `2026-08-26-cloud-deployment-smoke-opt-in-design.md`
25. `2026-08-10-intelligent-travel-planner-application-design.md`
26. `2026-08-10-visual-workflow-scheduler-design.md`
27. `2026-08-22-workflow-product-completion-design.md`
28. `2026-08-22-workflow-smtp-only-email-design.md`
29. `2026-08-10-owner-registration-control-design.md`
30. `2026-08-10-part18-persistent-ops-login-design.md`
31. `2026-08-10-part18-unified-ops-dual-view-design.md`
32. `2026-08-09-part18-multi-runtime-ops-correction-design.md`
33. `2026-08-09-part18-skill-runtime-and-server-ops-design.md`
34. `2026-07-29-part17-runtime-reliability-diagnostics-and-deployment-design.md`
35. `2026-07-27-web-product-experience-and-vue-frontend-design.md`
36. `2026-07-26-full-session-context-engineering-design.md`
37. `2026-07-24-weixin-clawbot-channel-design.md`
38. `2026-07-23-qq-external-channel-boundary-design.md`
39. `2026-07-21-subagent-runtime-boundary-design.md`
40. `2026-07-20-hook-runtime-boundary-design.md`
41. `2026-07-17-mcp-tool-runtime-boundary-design.md`
42. 按需阅读其它日期设计记录，理解某次改动的背景和权衡。

## 日期设计记录索引（非穷举）

日期设计文件是工作留痕，不要求本索引逐文件同步；需要完整清单时直接查看 `docs_design/2026-*.md`。当前能力判断始终回到上面的总体设计与 Part 活文档。

- `2026-08-26-weather-discovery-and-chat-history-presentation-fix-design.md`
- `2026-08-26-cloud-deployment-smoke-opt-in-design.md`
- `2026-08-26-ctrip-login-state-convergence-design.md`
- `2026-08-26-travel-finalizer-transport-source-reconciliation-design.md`
- `2026-08-26-travel-finalizer-server-draft-repair-design.md`
...
- `2026-08-10-owner-registration-control-design.md`
- `2026-08-09-admin-overview-drilldown-design.md`
- `2026-08-09-ops-visual-consistency-design.md`
- `2026-08-09-local-ops-button-feedback-design.md`
- `2026-08-09-local-ops-terminal-color-restoration-design.md`
- `2026-08-09-daily-jsonl-log-filename-design.md`
- `2026-08-09-part18-multi-runtime-ops-correction-design.md`
- `2026-08-09-part18-skill-runtime-and-server-ops-design.md`
