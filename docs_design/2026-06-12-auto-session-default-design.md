...
> 说明：这是一份已经被后续会话命令方案取代的实验记录。当前代码并不采用“每次启动都新建会话”的行为，而是保留稳定默认会话 `default`，并通过 `/new` 显式新建会话。

## 背景

当前 CLI 在用户未传入 `--session` 时，默认使用固定的 `default` 会话。这个行为是当前代码的真实状态，也正是后续 `/new`、`/reset`、`/sessions` 方案所建立的基础。
...
- 这条实验方案已经被撤回。
- 当前代码改为默认使用 `default`，并由 `/new` 生成新会话。
