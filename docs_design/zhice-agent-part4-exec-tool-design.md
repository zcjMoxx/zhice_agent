...
> 参考规范：`AGENTS.md`
>
> 当前状态：Milestone 3 安全 `exec` 已实现，并继续经过 RBAC、明确确认、Hook、workspace guard、危险命令拦截、超时与输出截断。

---

## 1. 第四部分当前交付

第四部分交付“安全 `exec` 工具”，使 ZhiCe-Agent 能在 workspace 和策略边界内执行测试、lint、只读 Git 检查与项目脚本。
...
规范要求破坏性操作必须能从用户请求中明确确认，不能由模型自行猜测执行。Part 4 初版尚无“暂停并等待用户确认某个 tool call”的交互协议；当前代码已在 Tool 执行策略层补齐 Web/WS 确认、权限、Hook 与审计，AgentLoop 仍不硬编码破坏性业务判断。
...
后续用户权限系统进入主线后，`exec` 的危险能力也不应直接变成“管理员可任意执行”。更稳的方向是：

```text
用户权限允许进入高风险执行路径
  -> runtime 展示精确命令并要求用户明确确认
  -> exec 校验确认上下文
  -> 执行结果和拒绝结果都写入审计
```

因此第四部分的一刀切危险命令拦截是当前安全底线；未来会在此之上叠加权限、确认和审计，而不是删除 workspace guard、超时、输出截断和危险命令识别。
...
这不是说这些能力永远不能做，而是它们应该经过更明确的用户授权、外部网络策略、运行日志和审计记录。第四部分先把本地安全执行闭环做好；后续用户权限系统会在 turn 与日志边界稳定后单独设计。
