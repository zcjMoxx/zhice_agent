...
> 说明：本记录落地后发现XHS Cookie模式仍保留了多余Authorization字段，且空的可选环境变量会使整个MCP目录失效。当前方案先由《2026-08-25 MCP配置隔离与XHS Cookie收口设计》收口，再由《2026-08-25 手机注册页单屏适配与XHS本地回环配置设计》将本地allowlist改为显式回环地址；本文正文保留当时方案原貌。
