...
def definition():
    return WorkflowDefinitionV1(
        ...
        nodes=(WorkflowNode("trigger", "schedule_trigger"), WorkflowNode("query", "mcp_query", config={"tool_name": "mcp__open-meteo__get_forecast"}), WorkflowNode("qq", "qq_notification", config={"send_consent_at": "2026-08-22T00:00:00Z"})),
        edges=(WorkflowEdge("e1", "trigger", target_node_id="query"), WorkflowEdge("e2", "query", target_node_id="qq")),
    )
...
def test_compile_and_roundtrip_restricted_flow():
    ...
    assert [node.type for node in restored.nodes] == ["schedule_trigger", "mcp_query", "qq_notification"]
